...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const internalAcknowledgement = input.kind === 'acknowledgement' && Boolean(serviceKey) && authorization === 'Bearer ' + serviceKey;
    const userClient = createClient(url, internalAcknowledgement ? serviceKey : Deno.env.get('SUPABASE_ANON_KEY')!, { global: { headers: { Authorization: authorization } }, auth: { persistSession: false } });
    if (!internalAcknowledgement) {
    ...
    if (!['receipt', 'decision', 'acknowledgement'].includes(input.kind)) return reply(400, { error: 'Invalid kind' });
    ...
    if (input.kind === 'acknowledgement') {
      ...
      if (error || !request || request.request_type !== 'membership') return reply(400, { error: 'Membership request required' });
      recipient = request.email;
      key = 'acknowledgement-' + request.id;
      ({ subject, text } = acknowledgementEmail(request, contactEmail));
      ...
      ({ subject, text } = decisionEmail(request, contactEmail));
    } else {
      ...
      ({ subject, text } = receiptEmail(receipt, contactEmail));
      attachments = [await receiptPdf(receipt)];
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient) || recipient.length > 320) return reply(400, { error: 'Valid email required' });
    const admin = createClient(url, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!, { auth: { persistSession: false } });
    const proposed = { attachments, from, to: [recipient], subject, text, html: '<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;padding:24px;line-height:1.6;white-space:pre-wrap">' + escapeHtml(text) + '</div>' };
